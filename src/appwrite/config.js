...
const { data_id, collection_id, endpoint, project_id, bucket_id } =
  AppwriteConfig;
export class Service {
  ...
  constructor() {
    this.client.setEndpoint(endpoint).setProject(project_id);
  ...
  async createPost(title, slug, content, featuredImage, userId, status) {
    try {
      return await this.databases.createDocument(data_id, collection_id, slug, {
        title,
        content,
        status,
        featuredImage,
  ...
  async updatePost(slug, { title, content, featuredImage, status }) {
    try {
      return await this.databases.updateDocument(data_id, collection_id, slug, {
        title,
        content,
        featuredImage,
        status,
